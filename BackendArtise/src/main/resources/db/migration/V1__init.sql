...
CREATE TABLE `artise`.`user_service` ( `id` INT NOT NULL AUTO_INCREMENT , `user_id` INT NOT NULL , `service_id` INT NOT NULL , `time` DATETIME NOT NULL , `is_cancelled` BOOLEAN NOT NULL DEFAULT FALSE , `is_confirmed` BOOLEAN NOT NULL DEFAULT FALSE , `price` FLOAT NOT NULL , PRIMARY KEY (`id`));
...
ALTER TABLE `notes` ADD FOREIGN KEY (`user_service_id`) REFERENCES `user_service`(`id`) ON DELETE RESTRICT ON UPDATE RESTRICT;
